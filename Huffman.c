...
void  tri_tableau(int ,elem tab[]); /* Tri le tableau de caracteres en fonction de leurs occurances*/
  ...
  if(choix==1)
    {
      ...
      printf("Attention, ce fichier ne doit contenir AUCUN caratère accentué !\n");
    ...
    {
      printf("******************************************************************************************\n");
      printf("Attention ! Partie non fonctionnelle car partiellement implémantée ! Veuillez nous excuser\n");
      printf("******************************************************************************************\n");
      char nom_du_huff[30];
      printf("Attention, ce fichier doit être en .huff !\n");
      printf("Indiquer le nom du fichier a décompresser: ");
      scanf("%s",nom_du_huff);
      FILE* origine=NULL;
      ...
      if(longeur==0)
	{
      ...
      int caractere;
      FILE* fin;
      fin=fopen("texte_court.txt.huff","r+");
      FILE* tmp_file=NULL;
      ...
      while(cryptage(fin))
	{
	  for(i=0;i<(sizeof(char)*8);i++)
	    fprintf(tmp_file,"%d",(caractere>>i)&1);
	  caractere=fgetc(fin);
	}
      fclose(tmp_file);
      ...
      while(cara_comp!=EOF)
	{
	  ...
	  while(caractere!=EOF)
	      ...
	      else 
		{
	      ...
	      if(strcmp(code_dico,code_bin)==0)
		{
...
void convertir_nombre(int nombre,FILE* fichier)
{
  if(nombre==0)
    return;
  convertir_nombre(nombre/2,fichier);
  fprintf(fichier,"%d",nombre%2);
}

/* Ecriture du dichier dictionnaire*/
...
void compresser(FILE* origine,char* nom_du_fichier)
{
  ...
  for(i=taille-1;i>=0;i--)                      /*Créations de feuilles contenant un caractere et son occurence chacune*/
    noeud[i]=creer_noeud(tab[i].oc,tab[i].car);/************************************************************************/
  /*********************************************************************************************************************/
